...
read_places <- 
  function(suf = suffix_regex()) {
   ...
   if (!file.exists("./data/placenames_nl.RData")) {
      ...
      write.table(my.table, "/data/placenames_nl.csv", sep="~")
...
make_maps <- 
  ...
  nl_hex_map <- ggplot2::fortify(hex_polys_nl)
  ...
  lapply(1:length(suf_nam), function(i) {
    ...
    gg <- ggplot()
    gg <- gg + geom_map(data=nl_hex_map, map=nl_hex_map,
                        aes(x=long, y=lat, map_id=id),
                        size=0.6, color="#ffffff", fill=no_fill)
    gg <- gg + geom_map(data=cur_heat, map=nl_hex_map,
